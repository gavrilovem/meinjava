public class Trapeze extends QGon {
    public Trapeze(Point2D[] p) {
        if (p.length == 4) {
            double k1 = (p[0].getX(1) - p[1].getX(1)) / (p[0].getX(0) - p[1].getX(0));
            double k2 = (p[2].getX(1) - p[3].getX(1)) / (p[2].getX(0) - p[3].getX(0));
    ...
    public double square() {
        ...
        double b = Math.sqrt(Math.pow(Point.sub(p[2], p[3]).getX(0), 2) + Math.pow(Point.sub(p[2], p[3]).getX(1), 2));
        if (a > b) { a = a + b; b = a - b; a = a - b; } // swap(a, b)
        double c = Math.sqrt(Math.pow(Point.sub(p[1], p[2]).getX(0), 2) + Math.pow(Point.sub(p[1], p[2]).getX(1), 2));
